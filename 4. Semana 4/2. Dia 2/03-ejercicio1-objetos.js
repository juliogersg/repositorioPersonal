...
console.log(`Cantidad de peliculas en el idioma solicitado: ${peliculasPorIdioma('ko')}`);

/**
 * Funcion que imprime el nombre de la pelicula con mas generos
 * y la cantidad de generos que tiene
 * 
 * Reto:
 * Mostrar la cantidad de peliculas que tienen la mayor cantidad de generos
 * en caso 
 */
function conMasGeneros(){
    let max = 0;
    for(let i=0;i<peliculas.length;i++){
        if(peliculas[i].genre_ids.length > peliculas[max].genre_ids.length){
            max = i;
        }
    }
    console.log(`La pelicula con mas generos es: ${peliculas[max].original_title}`);
    console.log(`La pelicula tiene: ${peliculas[max].genre_ids.length} generos`);
}
